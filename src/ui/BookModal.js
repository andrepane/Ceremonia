...
function getDedicationFont(guestId) {
...
function renderCoupleGuestbookContent(entries = []) {
  ...
  return entries
    .map((entry) => {
      const fontFamily = getDedicationFont(entry.fromGuestId);
      return `<article class="book-modal__dedication"><p class="book-modal__dedication-from" style="font-family:${fontFamily}">De: ${entry.fromName || "Invitado"}</p><p class="book-modal__dedication-text" style="font-family:${fontFamily}">${entry.content || ""}</p></article>`;
    })
    .join("");
}
...
export class BookModal {
  ...
  applyGuestFont(guestId) {
    const fontFamily = getDedicationFont(guestId);

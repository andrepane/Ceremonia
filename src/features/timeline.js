...
export function renderTimeline() {
  ...
  document.getElementById("timeline").innerHTML = timelineItems
    .map(({ item, index }) => {
      const isFridayDinner = index === 1;
      const isSaturdayBreakfast = index === 2;
      const isSaturdayLunch = index === 4;
      const isSaturdayPrivateDinner = index === 7;
      const isSundayBreakfast = index === 9;
